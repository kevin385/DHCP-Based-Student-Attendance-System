...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		if(empty($_POST["student_subject_id"]))
		{
			$error_student_subject_id = "Subject is required";
			$error++;
		}
		else
		{
		...
		if($error > 0)
		{
			$output = array(
				...
				'error_student_subject_id'		=>	$error_student_subject_id
			);
		}
		else
		{
			if($_POST["action"] == 'Add')
			{
				...
				(student_name, student_roll_number, student_dob, student_subject_id) 
				VALUES (:student_name, :student_roll_number, :student_dob, :student_subject_id)
				";
	...
	if($_POST["action"] == "edit_fetch")
	{
		...
		if($statement->execute())
		{
			...
			foreach($result as $row)
			{
				...
				$output["student_subject_id"] = $row["student_subject_id"];
