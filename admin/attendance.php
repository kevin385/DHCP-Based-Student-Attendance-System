...
<div class="modal" id="chartModal">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">Create Subject Attandance Chart</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      ...
      <div class="modal-body">
        <div class="form-group">
          <select name="chart_subject_id" id="chart_subject_id" class="form-control">
            <option value="">Select Subject</option>
            <?php
            echo load_subject_list($connect);
            ?>
          </select>
          <span id="error_chart_subject_id" class="text-danger"></span>
        </div>
        <div class="form-group">
          <div class="input-daterange">
...
$(document).ready(function(){
  ...
  $('#create_report').click(function(){
    ...
    if(subject_id == '')
    {
      $('#error_subject_id').text('Subject is Required');
      error++;
    }
    else
    {
    ...
    if(error == 0)
    {
      ...
      window.open("report.php?action=attendance_report&subject_id="+subject_id+"&from_date="+from_date+"&to_date="+to_date);
    }

  });

  $('#chart_button').click(function(){
    $('#chart_subject_id').val('');
    $('#attendance_date').val('');
    $('#chartModal').modal('show');
  });

  $('#create_chart').click(function(){
    var subject_id = $('#chart_subject_id').val();
    var attendance_date = $('#attendance_date').val();
    var error = 0;
    if(subject_id == '')
    {
      $('#error_chart_subject_id').text('Subject is Required');
      error++;
    }
    else
    {
      $('#error_chart_subject_id').text('');
    }
    if(attendance_date == '')
    {
    ...
    if(error == 0)
    {
      ...
      $('#chart_subject_id').val('');
      $('#chartModal').modal('show');
      window.open("chart1.php?action=attendance_report&subject_id="+subject_id+"&date="+attendance_date);
    }
